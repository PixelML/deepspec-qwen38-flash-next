"""Target-hidden-state cache preparation for Qwen/Qwen3.8-Flash-Next (qwen4_exp).

This is a COPY of scripts/data/prepare_target_cache.py (kept verbatim in
structure) with qwen4_exp-specific patches, per the DFlash Qwen3.8-Flash-Next
port contract (see docs/dflash-training-log.md for the full record):

1. `_get_target_backbone` / `_get_target_hidden_size` gain a qwen4_exp branch
   (composite config: real fields live under `.text_config`, and the loaded
   AutoModel exposes the text backbone at `.language_model`, mirroring the
   existing gemma4 branch).
2. `run_target_forward_with_hooks` is HC-aware for qwen4_exp targets: Qwen4Exp
   decoder layers operate on a `hc_count * hidden_size`-wide (4 * 2560 =
   10240) hyper-connection stream, NOT the native 2560-wide residual DeepSpec's
   cache format expects. Hooking the decoder layer itself (as the original
   script does) would capture the wrong (un-contracted) tensor. Instead we
   hook the FOLLOWING layer's `attn_hyper_connection` submodule and capture
   its first return value (`mixed_input`), which reuses the model's own
   learned HC-contraction weights and is semantically consistent with
   DeepSpec's own `extract_context_feature` convention of treating index
   `layer_id + 1` as "the hidden state after layer_id" (i.e. the input to
   layer_id + 1). For a target_layer_id that is the LAST decoder layer index
   (no `layer_id + 1` layer exists), we instead hook the model's final
   `hyper_connection_mixer` (same contraction math, `use_combine=False`,
   returns a plain tensor rather than a 3-tuple -- `_get_hook_tensor` already
   handles both shapes).
3. `target_model_revision` (an optional pinned commit SHA) is threaded through
...
def _get_target_backbone(target_model):
    ...
    if model_type in QWEN4_EXP_MODEL_TYPES:
        if hasattr(target_model, "language_model"):
            return target_model.language_model
        if hasattr(target_model, "model") and hasattr(target_model.model, "language_model"):
            return target_model.model.language_model
        assert False, "Qwen4Exp target model must expose a text language_model."
...
def run_target_forward_with_hooks(
    *,
    ...
    model_type = str(target_model.config.model_type)
    is_hc_model = model_type in QWEN4_EXP_MODEL_TYPES
    ...
    try:
        ...
        for layer_id in target_layer_ids:
            if layer_id < 0:
                continue
            if is_hc_model:
                # Native (contracted) width only exists momentarily inside
                # each layer's *_hyper_connection.forward(). "After layer_id"
                # == the input mix consumed by layer_id + 1's attention
                # sub-block (see module docstring). At the boundary (last
                # decoder layer), fall back to the model's final
                # hyper_connection_mixer, which performs the same
                # contraction on the same hyper-stream.
                if layer_id + 1 < len(layer_modules):
                    hook_module = layer_modules[layer_id + 1].attn_hyper_connection
                else:
                    hook_module = backbone.hyper_connection_mixer
                handles.append(hook_module.register_forward_hook(capture_layer(layer_id)))
            else:
                handles.append(
    ...
    manifest = build_target_cache_manifest(
        ...
        extra_fields={
            ...
            "target_model_revision": (
                str(config.model.get("target_model_revision"))
                if config.model.get("target_model_revision") is not None
                else None
            ),
